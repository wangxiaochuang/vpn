# VPN 架构文档

## 1. 概述

基于 QUIC 的点对点 VPN。客户端与服务端各创建一个 TUN 设备，应用流量经由 TUN 被 VPN 拦截，通过 QUIC 连接在两端之间中转。

核心设计：**控制面用 QUIC stream，数据面用 QUIC datagram**。stream 提供可靠有序的信令通道，datagram 提供低延迟的数据通道（IP 包本身可丢，无需重传）。

## 2. 架构总览

```
                    ┌─────────────────────────────────────────────┐
                    │              QUIC 连接 (TLS 1.3)             │
        控制面 ──── │  ┌─────────────────────────────────────┐    │
        (stream)    │  │ 认证 → IP 分配 / 配置下发 / 心跳     │    │
                    │  └─────────────────────────────────────┘    │
        数据面 ──── │  ┌─────────────────────────────────────┐    │
      (datagram)    │  │ 原始 IP 包 (TUN ↔ TUN，原样转发)     │    │
                    │  └─────────────────────────────────────┘    │
                    └─────────────────────────────────────────────┘
                            ▲                                ▲
                            │                                │
            ┌───────────────┴────────┐      ┌────────────────┴──────────────┐
            │       客户端            │      │            服务端             │
            ├────────────────────────┤      ├───────────────────────────────┤
            │  TUN: 10.0.0.2 (本机)  │      │  TUN: 10.0.0.1 (本机)         │
            │     ↓ 系统路由         │      │      ↓                        │
            │  普通应用流量 → TUN    │      │  收到包 → 写入 TUN → OS 转发   │
            │     ↓                  │      │      ↓                        │
            │  发 datagram 给服务端  │      │  OS 路由回包 → TUN → datagram  │
            └────────────────────────┘      │      ↓                        │
                                          │  发 datagram 给客户端          │
                                          │                               │
                                          │  NAT / IP forwarding (OS 层)  │
                                          └───────────────────────────────┘
```

## 3. 控制面（QUIC Stream）

- **职责**：可靠、有序的信令传输。
- **承载内容**：认证握手、虚拟 IP 分配、配置下发、心跳/保活。
- **消息格式**：Protobuf（`prost`）。
- **典型流程**：连接建立后客户端打开第一条 stream，发送认证请求；服务端校验后下发分配结果。

## 4. 数据面（QUIC Datagram）

- **职责**：低延迟地搬运 IP 包。
- **载荷**：一个完整的原始 IP 包，**原样装入 datagram，不做任何加工**（不加分片、不加 session 前缀）。
- **可靠性**：由 IP 层语义决定。datagram 丢就丢，对应 IP 包丢失，上层协议自行处理。
- **会话标识**：由 QUIC 连接本身标识，无需在 datagram 里再加 session ID。
- **MTU**：V1 不做处理。后续若遇到超过 QUIC datagram 上限的包，再考虑调整 TUN 的 MTU。

## 5. 认证与身份

分两层：

```
┌──────────────────────────────────────────────────────────┐
│ TLS 层: 自签证书 + 指纹固定 (SPKI pin)                   │
│   - 提供通道加密                                         │
│   - 防止 MITM 截获应用层凭证                             │
│   - 客户端配置里写死服务端证书的 SHA256 指纹              │
├──────────────────────────────────────────────────────────┤
│ 应用层: 用户名 / 密码                                    │
│   - 服务端配置文件存储用户列表                           │
│   - 密码使用 argon2 哈希存储                             │
│   - username 同时作为持久身份 (见 §6)                    │
└──────────────────────────────────────────────────────────┘
```

身份链路：

```
username  ──门禁──►  能不能进 (密码对不对)
username  ──身份──►  你是哪一个
username  ──资源──►  你该拿哪个虚拟 IP (持久映射)
```

## 6. IP 分配

- **地址池**：服务端配置一个固定 subnet（如 `10.0.0.0/24`），依次分配。
- **持久映射**：`username → 虚拟 IP`。映射在服务端持久化（即使客户端掉线，映射保留一段时间 lease）。
- **重连同 IP**：客户端掉线重连后，凭同一 `username` 取回原 IP。
- **多设备**：V1 不支持同一 username 多设备同时在线，不区分设备。

## 7. 服务端转发（TUN + OS NAT）

依赖操作系统级的网络转发：

```
 方向1 (上行): 客户端 → 外网
   datagram 收到 → 写入 TUN → OS (IP forwarding + NAT) → 公网出口

 方向2 (下行): 外网 → 客户端
   OS 收到回包 → 路由到 TUN → 服务端读出 → 查路由表 → 发 datagram
```

**服务端运行时核心数据结构 —— 路由表**：

```
   虚拟IP ──→ QUIC 连接句柄
   10.0.0.2 ──→ conn_A
   10.0.0.3 ──→ conn_B
```

服务端读到 TUN 的包后，按目标虚拟 IP 查表，找到对应的 QUIC 连接，发 datagram 过去。连接的建立、断开、重连、IP 释放都围绕这张表进行。

**OS 配置要求**（V1 由文档说明，用户手动配置）：
- 开启 IP forwarding
- 为 TUN subnet 配置 NAT 规则

## 8. 连接生命周期

```
 建立:
   QUIC 握手 (TLS) → 开控制 stream → 客户端发 {username, password}
     → 服务端校验 → 查 username→IP → 分配/复用 IP
     → 下发 {assigned_ip, subnet, server_ip}
     → 客户端配置 TUN → 双向数据泵启动

 断线:
   连接断开 → IP 映射保留 (lease) → 等待重连

 重连:
   同一 username 重连 → 取回原 IP → 复用映射
```

## 9. 配置形态（示意）

服务端：

```toml
[server]
listen = "0.0.0.0:443"
tun_subnet = "10.0.0.0/24"

[[users]]
username = "alice"
password_hash = "$argon2..."   # argon2 哈希
```

客户端：

```toml
[client]
server = "vpn.example.com:443"
server_cert_fingerprint = "sha256/..."   # 证书指纹固定
username = "alice"
password = "..."                         # 或交互输入
```

## 10. 技术栈

| 组件 | 选型 |
|------|------|
| QUIC | `quinn` |
| TLS | `rustls` (aws-lc-rs) |
| TUN 设备 | `tun-rs` (async) |
| 异步运行时 | `tokio` |
| 消息序列化 | `prost` (protobuf) |
| CLI | `clap` |
| 证书生成 | `rcgen` |
| 密码哈希 | `argon2` |

## 11. V1 范围与非目标

**V1 包含**：
- 用户名/密码认证（服务端配置存储，argon2 哈希）
- 单端口、单 subnet 的 IP 分配
- username → IP 持久映射，重连同 IP
- TUN + OS NAT 转发
- 自签证书 + 指纹固定

**V1 不包含（后续迭代）**：
- MTU 协商 / 分片处理
- 同一 username 多设备同时在线
- 服务端自动配置 NAT 规则
- 更复杂的认证（如 token、证书认证、MFA）
- 流量统计 / 计费

## 12. 决策记录

| 决策 | 理由 |
|------|------|
| 控制面 stream / 数据面 datagram | 兼顾信令可靠性与数据面低延迟 |
| 应用层用户名密码（非 TLS-PSK） | 简单直观，username 天然作身份 |
| argon2 哈希存储密码 | 配置泄露时不暴露明文，成本极低 |
| username 作为持久身份 | 自然解掉"重连同 IP"的身份识别问题 |
| datagram 装原始 IP 包原样转发 | 最简实现，QUIC 连接已标识会话 |
| TUN + OS NAT | 主流方案，性能好，用户态不重 |
| 证书指纹固定 | 防止 MITM 截获应用层凭证 |
