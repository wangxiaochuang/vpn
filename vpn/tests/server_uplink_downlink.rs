...
use tokio::sync::mpsc;
use vpn::data::downlink_pump;
use vpn::server::RegistryDispatcher;
use vpn::server::spawn_uplink_task;
...
async fn test_spawn_uplink_task_accepts_mock_packet_sink_without_tun() {
    ...
    let mut tasks: tokio::task::JoinSet<vpn::server::ConnExitCause> = tokio::task::JoinSet::new();
...
async fn test_uplink_via_spawn_task_exits_on_connection_close() {
    ...
    assert!(matches!(cause, Ok(vpn::server::ConnExitCause::UplinkEnded)));
}
