# Client Config Delta Specification

本 delta 引入新的 `client-config` capability：客户端配置文件解析与语义校验（server / server_name / ca_cert / username，密码交互输入）。解析器复用 `ServerConfig::load` 的 TOML + 语义校验模式，`ConfigError` 扩展客户端专用变体。
