# QUIC Link Datagram Specification

## Purpose

定义 `quic-link` crate 中基于 QUIC datagram 的包收发契约：`PacketSource` / `PacketSink` trait 抽象、跨 trait 双向转发的通用泵 `forward`，以及基于 quinn datagram 的具体实现 `DatagramTx` / `DatagramRx`。本 spec 是 `quic-link` datagram 模块 Q1/Q2 测试的契约来源。
