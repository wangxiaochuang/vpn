# QUIC Link Streams Specification

## Purpose

定义 `quic-link` crate 中基于 QUIC bidi stream 的类型化消息通道契约：`Session` 的 `open_stream` / `accept_stream` 显式开启、同一 Session 多 stream 多路复用、stream 类型由调用方协议约定（非 QUIC 保证）、以及防死等的超时配合方式。
