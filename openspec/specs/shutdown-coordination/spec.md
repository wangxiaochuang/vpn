...
定义 `shutdown` crate 的优雅关闭协调能力契约：`Shutdown` 取消根（持有 `CancellationToken` 与 drain 超时，广播取消并 drain `JoinSet`）、`ShutdownHandle`（worker 侧取消句柄）、信号 watchdog（提前注册 SIGINT/SIGTERM 并 ready 握手）、内联中断等待 `wait_for_interrupt`。本 capability 是 `shutdown` crate 的 Q1 单元测试与 Q2 场景测试契约来源。`shutdown` SHALL NOT 持有任何业务/传输资源，SHALL NOT 关心调用方如何编排 close 顺序，SHALL NOT 将内部 `CancellationToken` 类型暴露给调用方。
