...
定义控制面 framing 的**薄适配**能力契约：以 `ControlCodec`（`msgx::ProtoCodec<ControlMessage>` 的类型别名）复用 `msgx` crate 的泛型 protobuf framing（字节边界：4 字节大端长度前缀、`MAX_FRAME_LENGTH` 上限、半包/粘包内置）与 `prost`（消息编解码），使上层 `Framed<RecvStream, ControlCodec>` 直接吞吐 `ControlMessage`。framing 的字节边界逻辑由 `msgx::ProtoCodec`（内部 `LengthDelimitedCodec`）承担，本 capability 不重写；本 spec 聚焦 `ControlCodec` 对 `msgx::ProtoCodec` 的复用契约。`MAX_FRAME_LENGTH` 与 `ControlMessage` 定义于 `control-plane` / `msgx` capability，本 capability 引用它们。`Encoder`/`Decoder` 操作 `BytesMut`、为同步纯逻辑（无 `AsyncRead`），是 `framing` 模块的 Q1 单元测试契约来源；IO 接入（`Framed` 包装 `quinn` stream、心跳循环）不在范围。
